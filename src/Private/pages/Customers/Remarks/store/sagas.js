...
export const getRemarks = takeEvery(actions.GET_REMARKS, function* (action) {
    try {
        const res = yield call(api.get, `customer/${action.customer_id}/remarks`, action.query);
        yield put({
...
export const getRemarksById = takeEvery(actions.GET_REMARKS_BYID, function* (action) {
    try {
...
export const createRemarks = takeEvery(actions.CREATE_REMARKS, function* (action) {
    try {
        const res = yield call(api.post, `customer/${action.customer_id}/remarks`, action.data);
        yield put({
