...
import { organizationStakeholderStatusOptions } from "../data/stakeholderStatus";
...
export default function ChangeOrganizationStakeholderStatusModal({
    open,
    onClose,
    stakeholderId,
    onSuccess,
    ...
    const { loading: isLoading, success: isSuccess } = useSelector(
        (state) => state.change_organization_stakeholder_status,
    );

    const dispatch = useDispatch();

    const handleSubmit = (data) => {
        dispatch(stakeholderActions.change_organization_stakeholder_status(stakeholderId, data));
    };
    ...
    useEffect(() => {
        if (isSuccess) {
            methods.reset();
            dispatch(stakeholderActions.change_organization_stakeholder_status_reset());
            onSuccess?.();
            onClose?.();
        }
    }, [isSuccess]);

    const statusOptions = organizationStakeholderStatusOptions.filter(
...
ChangeOrganizationStakeholderStatusModal.propTypes = {
