...
export const getPaymentRules = takeEvery(
    actions.GET_PAYMENT_RULES,
    function* (action) {
        try {
...
export const getPaymentRulesDetailById = takeEvery(
    actions.GET_PAYMENT_RULES_DETAILS_BY_ID,
    function* (action) {
        try {
...
export const addPaymentRules = takeEvery(
    actions.ADD_PAYMENT_RULES,
    function* (action) {
        try {
...
export const updatePaymentRules = takeEvery(
    actions.UPDATE_PAYMENT_RULES,
    function* (action) {
        try {
...
export const updatePaymentRuleStatus = takeEvery(
    actions.UPDATE_PAYMENT_RULES_STATUS,
    function* (action) {
        const query = api.getJSONToQueryStr(action.data);
        try {
            const res = yield call(
                api.patch,
                `compliance/${action.id}?${query}`
            );
            yield put({
...
export const deletePaymentRules = takeEvery(
    actions.DELETE_PAYMENT_RULES,
    function* (action) {
        try {
