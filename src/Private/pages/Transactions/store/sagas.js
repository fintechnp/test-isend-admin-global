...
export const getTransactions = takeEvery(actions.GET_TRANSACTIONS, function* (action) {
    try {
...
export const getTransactionsByCustomer = takeEvery(actions.GET_TRANSACTIONS_BY_CUSTOMER, function* (action) {
    try {
        const res = yield call(api.get, `${action.id}/transaction`, action.query);
        yield put({
...
export const calculateTransaction = takeEvery(actions.CALCULATE_TRANSACTIONS, function* (action) {
    try {
...
export const createTransaction = takeEvery(actions.CREATE_TRANSACTIONS, function* (action) {
    try {
        const res = yield call(api.post, `transaction`, action.data);
        yield put({
...
export const updateTransaction = takeEvery(actions.UPDATE_TRANSACTIONS, function* (action) {
    try {
        const res = yield call(api.get, `transaction/${action.id}`, action.data);
        yield put({
