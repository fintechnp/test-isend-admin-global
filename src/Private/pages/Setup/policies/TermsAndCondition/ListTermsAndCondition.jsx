...
import AddTermsAndConditionsContent from "./TermsAndConditionContents/AddTermsAndConditionsContent";
import ViewTermsAndConditionsContent from "./TermsAndConditionContents/ViewTermsAndConditionsContent";
import UpdateTermsAndConditionsContent from "./TermsAndConditionContents/UpdateTermsAndConditionsContent";
...
const ListTermsAndCondition = () => {
    ...
    const TcDocumentData = TcDocumentList?.data ?? {};

    const TcDocumentSubData = TcDocumentList?.data?.contents ?? [];

    const fetchTCDocuments = () => {
        dispatch(
    ...
    const handleDeleteTitle = () => {
        confirm({
            ...
            description: "Are you sure you want to delete this Terms & Conditions?",
            confirmText: "Delete",
        }).then(() => {
            dispatch(
    ...
    const handleDeleteContent = (id) => {
        confirm({
            title: "Delete Terms & Condition Content?",
            description: "Are you sure you want to delete this Terms & Condition content?",
            confirmText: "Delete",
        }).then(() => {
    ...
    const TCColumn = [{ label: "Title", value: TcDocumentData.title ? TcDocumentData.title : "-" }];

    const HeaderDocumentTitle =
        TcDocumentData &&
        Object.keys(TcDocumentData).length > 0 &&
        TcDocumentData.title != null &&
        TcDocumentData.title !== ""
            ? TcDocumentData.title
            : "-";

    const IsEmptyData = isEmpty(TcDocumentData) && Object.keys(TcDocumentData).length === 0;
    ...
    return (
        ...
        >
            <Grid container spacing={4}>
                <Grid item xs={12} md={8}>
                    {isLoading ? (
                        ...
                        <DocumentFileTable rows={TCColumn} />
                    )}
                </Grid>
            </Grid>
            ...
            <Box sx={{ mt: 4 }}>
                ...
                <Grid item sx={{ mt: 2 }}>
                    {isError ? (
                        ...
                        <TanstackReactTable columns={columns} data={TcDocumentSubData} loading={isLoading} />
                    )}
                </Grid>
            </Box>
            ...
            <Column>
                <TablePagination
                    paginationData={TcDocumentList?.pagination}
...
export default withPermission({ permission: [permissions.READ_DOCUMENT_SETUP] })(ListTermsAndCondition);
