...
export const getAllServiceCharge = takeEvery(actions.GET_SERVICE_CHARGE, function* (action) {
    try {
...
export const getServiceChargeByPartner = takeEvery(actions.GET_SERVICE_CHARGE_BY_PARTNER, function* (action) {
    try {
        const res = yield call(api.get, `${action.id}/servicecharge`, action.query);
        yield put({
...
export const getServiceChargeDetails = takeEvery(actions.GET_SERVICE_CHARGE_DETAILS, function* (action) {
    try {
...
export const addServiceCharge = takeEvery(actions.ADD_SERVICE_CHARGE, function* (action) {
    try {
...
export const updateServiceCharge = takeEvery(actions.UPDATE_SERVICE_CHARGE, function* (action) {
    try {
        const res = yield call(api.put, `servicecharge/${action.id}`, action.data);
        yield put({
...
export const updateServiceChargeStatus = takeEvery(actions.UPDATE_SERVICE_CHARGE_STATUS, function* (action) {
    const query = api.getJSONToQueryStr(action.data);
    try {
        const res = yield call(api.patch, `servicecharge/${action.id}?${query}`);
        yield put({
...
export const deleteServiceCharge = takeEvery(actions.DELETE_SERVICE_CHARGE, function* (action) {
    try {
