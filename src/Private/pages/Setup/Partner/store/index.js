export { default as PartnerSaga } from "./sagas";
export { default as GetAllPartnerReducer } from "./reducers/getAllPartner";
export { default as GetPartnerDetailsReducer } from "./reducers/getPartnerDetails";
export { default as AddPartnerReducer } from "./reducers/addPartner";
export { default as UpdatePartnerReducer } from "./reducers/updatePartner";
export { default as DeletePartnerReducer } from "./reducers/deletePartner";

export { default as GetAllCorridorReducer } from "./reducers/getAllCorridor";
export { default as GetCorridorDetailsReducer } from "./reducers/getCorridorDetails";
export { default as AddCorridorReducer } from "./reducers/addCorridor";
export { default as UpdateCorridorReducer } from "./reducers/updateCorridor";
export { default as DeleteCorridorReducer } from "./reducers/deleteCorridor";
export { default as PartnerAction } from "./actions";
