...
export const getDeliveryRoute = takeEvery(
    actions.GET_DELIVERY_ROUTE,
    function* (action) {
        try {
...
export const getDeliveryRouteById = takeEvery(
    actions.GET_DELIVERY_ROUTE_BY_ID,
    function* (action) {
        try {
...
export const createDeliveryRoute = takeEvery(
    actions.CREATE_DELIVERY_ROUTE,
    function* (action) {
        try {
...
export const updateDeliveryRoute = takeEvery(
    actions.UPDATE_DELIVERY_ROUTE,
    function* (action) {
        try {
            const res = yield call(
                api.put,
                `deliveryroute/${action.id}`,
                action.data
            );
            yield put({
...
export const updateDeliveryRouteStatus = takeEvery(
    actions.UPDATE_DELIVERY_ROUTE_STATUS,
    function* (action) {
        const query = api.getJSONToQueryStr(action.data);
        try {
            const res = yield call(
                api.patch,
                `deliveryroute/${action.id}?${query}`
            );
            yield put({
...
export const deleteDeliveryRoute = takeEvery(
    actions.DELETE_DELIVERY_ROUTE,
    function* (action) {
        try {
